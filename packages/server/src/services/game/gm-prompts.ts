...
export function buildGmSystemPrompt(ctx: GmPromptContext): string {
  ...
  sections.push(
    ...
    `- Default to second person for narration and action. Narrate from their respective POV. Treat player input as a committed action or dialogue choice (see PLAYER INPUT below for when to render the player's speech-act inputs as quoted [main] lines). However, you ultimately decide whether their intent failed or succeeded, and you are free to reinterpret their input if necessary. Follow the principle of logic when deciding that.`,
