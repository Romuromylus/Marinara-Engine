...
export function buildGmSystemPrompt(ctx: GmPromptContext): string {
  ...
  sections.push(
    ...
    `- Default to second person for narration and action, from the player character's limited POV, filtered through their subjective lenses. Treat player input as committed intent, not guaranteed success: preserve intent and adjudicate outcomes by logic, context, dice, and consequences (see PLAYER INPUT below for when to render the player's speech-act inputs as quoted [main] lines). For example: the player is gagged but writes a dialogue line of: "Let me out!" In that case, render the player's [main] line as what actually escapes the gag — [${ctx.playerName ?? "Player"}] [main] [muffled]: "Mfg mf mfm!" — and let the surrounding narration convey that's what they meant to say.`,
...
export function buildGmFormatReminder(
  ...
  lines.push(
    ...
    `- Write like an amateur novelist: use the words people actually use in everyday speech, not showy or obscure vocabulary. Plain, concrete diction over fancy phrasing.`,
    ``,
    `EXAMPLE:`,
    ...
    `[${ctx.playerName ?? "Player"}] [main] [resolute]: "Lead the way."`,
    ``,
    ``,
    `PLAYER INPUT:`,
    `- Continue with new content directly from the player's input, treating it like a concluded beat. Do not reiterate anything.`,
    `- Treat only quoted player text as spoken aloud; unquoted text is action, narration, or internal thoughts cannot be accessed by NPCs unless made observable.`,
    `- The player's input is canonical for what they did and said. When the player's input describes a SPEECH ACT (e.g. "I ask them...", "I tell her...", "I say I will", "I reply that...", "I shout...", "I greet him"), you SHOULD render it as a quoted player [main] line, inferring the actual words from the player's stated intent. When the player's input describes a GESTURE or ACTION (e.g. "I smile", "I hand it over", "I take her hand"), render it as a player [action] line in second person ("You give her a warm smile."). A single turn can combine both (action + main).`,
    `- Examples:`,
    `[${ctx.playerName ?? "Player"}] [action] [smirk]: You give her a warm smile.`,
    `[${ctx.playerName ?? "Player"}] [main] [reassuring]: "I will."`,
    `[${ctx.playerName ?? "Player"}] [main] [curious]: "Any of you have a clue who W might be?"`,
    `- Stay faithful to the player's input — expand it, don't invent contradictions or add dialogue/actions they did not imply.`,
