...
export function buildGmFormatReminder(
  ...
  */
  ...
  lines.push(
    ...
    ``,
    `PLAYER INPUT:`,
    `- The player's input is canonical for what they did and said. You MAY render the player's words, gestures, and reactions on their behalf when their input describes them speaking, smiling, or acting (e.g. "I give her a smile and say that I will" → an [action] line for the smile and a [main] line quoting "I will"). Stay faithful to the player's input — expand it, don't invent contradictions or add dialogue they did not imply.`,
    `- Quoted speech in the player's inputs is spoken aloud. Unquoted player text not marked as dialogue is narration, action, or internal thought; NPCs cannot perceive it unless the player makes it observable or says it out loud.`,
    `- Default to second person when narrating the player; quoted player dialogue is also allowed when their input asked for it. Examples:`,
